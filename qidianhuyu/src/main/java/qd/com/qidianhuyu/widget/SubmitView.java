...
public class SubmitView extends AppCompatTextView implements TextWatcher {
    ...
    public void afterTextChanged(Editable s) {
        ...
        if (hasEmpty) {
            setBackground(getResources().getDrawable(R.drawable.bg_solid_round_color_disable_radius));
            setAlpha(0.7F);
            setEnabled(false);
        } else {
            setBackground(getResources().getDrawable(R.drawable.bg_solid_round_color_enable_radius));
            setAlpha(1.0F);
            setEnabled(true);
        }
    }
